...
public class RemoteFridgeAccess {
  ...
  private List<Food> getFreezerContent(){
    HttpRequest request =
          HttpRequest.newBuilder(makeUri("/getFreezerContent"))
  ...
  private List<Food> getFrigdeContent(){
    HttpRequest request =
          HttpRequest.newBuilder(makeUri("/getFridgeContent"))
              .header(ACCEPT_HEADER, APPLICATION_JSON).GET().build();
      try {
        final HttpResponse<String> response = HttpClient
            .newBuilder()
  ...
  private int getFridgeMaxsize(){
    HttpRequest request =
    HttpRequest.newBuilder(makeUri("/getFrigdeMaxsize"))
        .header(ACCEPT_HEADER, APPLICATION_JSON).GET().build();
    try {
      final HttpResponse<String> response = HttpClient
          .newBuilder()
  ...
  private int getFreezerMaxsize(){
    HttpRequest request =
    HttpRequest.newBuilder(makeUri("/getFreezerMaxsize"))
        .header(ACCEPT_HEADER, APPLICATION_JSON).GET().build();
    try {
      final HttpResponse<String> response = HttpClient
          .newBuilder()
