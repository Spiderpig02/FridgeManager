[![Gitpod Ready-to-Code](https://img.shields.io/badge/Gitpod-Ready--to--Code-blue?logo=gitpod)](https://gitpod.stud.ntnu.no/#https://gitlab.stud.idi.ntnu.no/it1901/groups-2022/gr2239/FridgeManager)
...
Dette prosjektet inneholder Java-kode og tester til disse. Det er delt opp i tre separate moduler: "core", "fxui" og "springboot".   
<u>**Core:**</u>  
...
For å kjøre prosjektet trengs det en server. Ettersom vi ikke har en server tilgjengelig bruker vi denne pcen som en "localhost", altså at den hoster sin egen server. For å få til dette bruker vi disse fire komandoene
```
cd FridgeManager/
mvn install
cd springboot
...
Videre for å kjøre prosjektet har man to muligheter. Åpne en *ny* terminal og skriv følgende tre kommandoer:
```
cd FridgeManager/
mvn install
...
Ved utvikling av nye features oppretter vi en ny branch ved navn «feat/navnpåfeature». Vi brancher denne ut fra et "issue" ved hjelp av gitlab, å det er tydelig hvilket "issue" branchen hører til. Dette er for å unngå konflikter i master, ettersom det er viktig at denne fungerer til enhver tid. Ved ferdigstillelse av feature merger vi først alle brancher utenom master inn i en, sjekker at denne funker, for så å merge denne inn i master. På denne måten minimerer vi risikoen for at master ikke vil fungere etter merging.  
...
I tillegg til disse er også jpackage og jlink implementer i POM-filen. Disse muliggjør såkalt "shippable product" altså at vi kan sende ut en .jar fil som kan brukes på andre maskiner og kjøre uten mvn og VSCode.